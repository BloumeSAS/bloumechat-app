...
function classifyActivity(
  ...
  if (MUSIC_PLAYERS.has(lc)) {
    ...
    return {
      type: "listening",
      name: NAME_MAP[lc] || prettifyName(processName),
    };
  }
...
function startHookProcess(
  ...
  child.on("exit", (code) => {
    ...
    if (ranMs < HOOK_EARLY_EXIT_MS) {
      hookEarlyExitCount++;
      console.warn(
        `[RPC] Hook process exited early (code ${code}, ran ${ranMs}ms) — attempt ${hookEarlyExitCount}/${HOOK_MAX_EARLY_EXITS}`,
      );
      if (hookEarlyExitCount >= HOOK_MAX_EARLY_EXITS) {
        console.warn(
          "[RPC] Falling back to single-shot polling for this session",
        );
  ...
  const onPollSettled = (result: "changed" | "blocked" | "unchanged") => {
    if (result === "unchanged") {
      ...
      currentIntervalMs = Math.min(
        ...
        MIN_POLL_INTERVAL_MS + stableStreak * BACKOFF_STEP_MS,
      );
    } else {
