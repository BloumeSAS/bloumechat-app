  ...
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (pathname === "/" || pathname === "") pathname = "/home/index.html";
        ...
        pathname = "/update/index.html";
      else if (pathname === "/screen-picker" || pathname === "/screen-picker/")
        pathname = "/screen-picker/index.html";
      else if (!pathname.includes(".")) {
        if (fs.existsSync(path.join(serveDir, pathname + ".html"))) {
          pathname += ".html";
        } else if (
          fs.existsSync(path.join(serveDir, pathname + "/index.html"))
        ) {
          pathname += "/index.html";
        }
      }
      ...
      if (
        filePath !== serveDirResolved &&
        !filePath.startsWith(serveDirResolved + path.sep)
      ) {
        res.writeHead(403);
        res.end("Forbidden");
        return;
    ...
    server.listen(PROD_PORT, "127.0.0.1", () => {
      console.log(
        `[Server] Production server running at http://127.0.0.1:${PROD_PORT}`,
      );
