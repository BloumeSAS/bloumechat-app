...
/** Resolves the language to render with, preferring the account's language
 * (synced from the webapp, and cached in electron-store across sessions —
 * see `window.ipc.getAccountLanguage()`) over the OS/Chromium locale. Falls
 * back to `detectLang()` (PC language) when nothing is logged in / cached yet. */
export function resolveLang(
  accountLang: string | null | undefined,
): SupportedLang {
  if (accountLang) {
    const code = normalize(accountLang);
    if (code in LOCALES) return code as SupportedLang;
  }
  return detectLang();
}
